...
start transaction;
select add_to_clienteParticular(44112233, 103103103,'Gustavo Lima','922887700', 'Rua Castanheira do Sul N11', 659486259);
commit;
...
set transaction isolation level repeatable read;
start transaction;
select add_to_clienteParticular(33112233, 123123123,'Raquel Lima','933887700', 'Rua Castanheira do Sul N11');
commit;
...
set transaction isolation level repeatable read;
start transaction;
select add_to_clienteParticular(33112233, 599667334,'Lurdes Campos','955443322', 'Rua Amendoeira do Norte N1');
commit;
...
set transaction isolation level repeatable read;
start transaction;
select add_to_clienteParticular(33223311, 123123123,'Lurdes Campos','955443322', 'Rua Amendoeira do Norte N1');
commit;
...
set transaction isolation level repeatable read;
start transaction;
select update_from_clienteParticular(88224466, 456456456, 'Núria Barros', '911223344','Rua Cajueiro Seco N30', 103103103, 854645646);
commit;
...
set transaction isolation level repeatable read;
start transaction;
select update_from_clienteParticular(66224466, 567567567, 'Samanta Alfredo', '933223344','Rua Avelã Damascena N10', 123123123);
commit;
...
set transaction isolation level repeatable read;
start transaction;
select update_from_clienteParticular(57203809, 135135135, 'Rita Duarte', '910000123','Rua das Nozes Douradas N14', 456456456); 
commit;
...
set transaction isolation level repeatable read;
start transaction;
select update_from_clienteParticular(11001100, 100220011, 'Joana Paulo', '933003300' ,'Rua dos Figos Secos N7', 999999999); 
commit;
...
start transaction;
select remove_from_clienteParticular(66224466);
commit;
...
set transaction isolation level read uncommitted;
start transaction;
select remove_from_clienteParticular(00000000);
commit;
...
select count_num_of_alarms(2022,'20-VT-65');
...
-- parâmetros: ano, matricula
select count_num_of_alarms(2022);
...
-- parâmetros: ano, matricula
select count_num_of_alarms(2020);
...
-- parâmetros: ano, matricula
select count_num_of_alarms(2022, 'ZZ-33-ZZ');
...
-- test 16: Verificar se as coordenadas do registo a ser processado se encontrarem dentro da zona verde: Resultado NOK
-- parâmetros: zonaVerdeLongitude, zonaVerdeLatitude, zonaVerdeRaio, processadoLongitude, processadoLatitude
select zonaVerdeValida(2.0, 1.8, 6.0, 1.5, 1.3);


-- test 17: Verificar se as coordenadas do registo a ser processado se encontrarem dentro da zona verde: Resultado OK
-- parâmetros: zonaVerdeLongitude, zonaVerdeLatitude, zonaVerdeRaio, processadoLongitude, processadoLatitude
select zonaVerdeValida(3.4, 2.9, 1.2, 2.0, 3.5);
...
-- parâmetros: zonaVerdeLongitude, zonaVerdeLatitude, zonaVerdeRaio, processadoLongitude, processadoLatitude
select zonaVerdeValida(3.4, 2.9, 1.2, 2.0);
...
select zonaVerdeValidaTest(10.4);


-- test 20: Verificar se o arredondamento da latitude do registo processado resulta num número par: Resultado NOK
-- parâmetros: processadoLatitude
select zonaVerdeValidaTest(10.7);


-- test 21: Analisar o registo processado aquando da sua criação e gerar o respetivo alarme (fora de qualquer uma zona verde): Resultado OK
start transaction;
